...
struct MC_Atom {
    struct MC_Atom *next;
    size_t length;
    // byte data is appended to each MC_Atom struct
};
static struct MC_Atom *buckets[8191] = { 0 };

//---------------------------------------------------------------------------//
size_t MC_AtomLength( const MC_Atom *atom ) {
    assert( atom );
    return atom->length;
}

//---------------------------------------------------------------------------//
const char* MC_AtomBytes( const MC_Atom *atom ) {
    assert( atom );
    return (const char *)(atom + 1);
}

//---------------------------------------------------------------------------//
unsigned int MC_AtomHash( const char *str, size_t len ) {
    unsigned int hash = 0;
    for ( size_t i = 0; i < len; i++ ) {
        hash = hash * 31 + scatter[ (unsigned char)str[ i ] ];
    }
    return hash;
}

//---------------------------------------------------------------------------//
const MC_Atom *MC_AtomNew( const char *str, size_t len ) {
    assert( str );
    assert( len >= 0 );
    unsigned int index = (MC_AtomHash( str, len ) % 8191);
    MC_Atom *atom = buckets[ index ];
    for ( ; atom; atom = atom->next ) {
        if ( len != atom->length ) {
            continue;
        }
        if ( memcmp( MC_AtomBytes(atom), str, len ) == 0 ) {
            return atom;
        }
    }
    atom = malloc( sizeof( MC_Atom ) + len + 1);
    atom->length = len;
    char * data = (char *)( atom + 1 );
    memcpy( data, str, len );
    data[len] = '\0';
    atom->next = buckets[ index ];
    buckets[ index ] = atom;
    return atom;
}

//---------------------------------------------------------------------------//
const MC_Atom *MC_AtomString( const char *str ) {
    assert( str );
    return MC_AtomNew( str, strlen( str) );
}

//---------------------------------------------------------------------------//
const MC_Atom *MC_AtomLong( long n ) {
    char str[256];
    sprintf(str, "%ld", n);
    return MC_AtomNew( str, strlen ( str ) );
}
...
#include "asrt.h"
//---------------------------------------------------------------------------//
static int TestAtomLength( const MC_Atom *atom, size_t len ) {
    ASRT_EQ( MC_AtomLength(atom), len );
    return 0;
}

//---------------------------------------------------------------------------//
static int TestAtomNew() {
    const MC_Atom *atomA = MC_AtomNew("bananaman", 5);
    const MC_Atom *atomB = MC_AtomNew("bananaman", 6);
    const MC_Atom *atomC = MC_AtomNew("bananaman", 5);
    ASRT_EQ( atomA, atomC );
    ASRT_FALSE( atomA == atomB );
    return 0;
}

//---------------------------------------------------------------------------//
void MC_AtomSuite() {
    ASRT_TEST( TestAtomLength, MC_AtomNew("bananaman", 5), 5 );
    ASRT_TEST( TestAtomLength, MC_AtomString("batman"), 6 );
    ASRT_TEST( TestAtomLength, MC_AtomLong(1234567890), 10 );
    ASRT_TEST( TestAtomNew );
}
